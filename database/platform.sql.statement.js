...
const __REFRESH_AUTHORIZER__ = 'UPDATE tb_authorizer SET accessToken = ?, expiresIn = ?, refreshToken = ?, funcInfo = ?, type = ? WHERE appid = ?';
...
const __FETCH_AUTHORIZER_INFO__ = 'SELECT a.* FROM tb_authorizer a, tb_user b, rel_authorizer_user c WHERE b.3rd_session = ? AND a.type = ? AND b.openid = c.openid AND c.appid = a.appid';
